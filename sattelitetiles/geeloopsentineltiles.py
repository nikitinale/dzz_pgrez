...
def get_sentinel2_tile(aoi: ee.Geometry.Polygon,
    ...
    print(
        f'Cloud density on the tail: {cloud_density}\nIndex of the tile: {tile_index}\nTime of shoot: {tile_time}\nID: {product_id}')
    if band_arr_b2.mean() > 1000:
        print('ATTENTION: CLOUD COVER HIGHLY PROBABLY UNFILTERED!')
        print(
            f'Means of B2, B3, B4, B8 bands: {band_arr_b2.mean()}, {band_arr_b3.mean()}, {band_arr_b4.mean()}, {band_arr_b8a.mean()}')
...
def extract_sentinel2_inside_geodataframe(data_collector: satteliteTiles,
                                          ...
                                          # of datetime.datetime [start, finish]
                                          dates: list,
                                          buff=20, square=320,
                                          columns=[]  # list of str
                                          ):
    ...
    buff=20
      minimal distance from a border of a parcel to the center of a tile 
      in units of crs (m)
    square=320
